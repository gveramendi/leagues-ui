...
export class RoleListComponent implements OnInit {
  ...
  private loadResourcesForRoles(): void {
    if (this.rows.length === 0) {
      this.loading = false;
      return;
    }

    const permissionRequests = this.rows.map((role) =>
      this.permissionService.getByRoleId(role.id)
    );

    forkJoin(permissionRequests).subscribe({
      next: (responses) => {
        responses.forEach((response, index) => {
          this.rows[index].resources = response.body.data.map((permission) => ({
